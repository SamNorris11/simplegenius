...
// Verified via the real Chromium renderer with realistic worst-case sentence
// text (every cell at the cap, all rows filled): at 5 rows, 200 chars/cell
// overflowed onto the footer by hundreds of pixels, and even 110 chars/cell
// still overlapped by ~80px at 5 rows. Fix has two parts, both required:
// cap rows at 4 (below) AND lower this char cap to 110 — that combination is
// the one confirmed safe (positive margin) via the same renderer test.
// Neither change alone is sufficient.
const COMPARISON_CELL_MAX_CHARS = 110;
