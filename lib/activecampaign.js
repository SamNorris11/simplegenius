 ...
 */
async function syncBriefDelivered({ email, firstName, lastName, pdfUrl }) {
  if (!getConfig()) {
  ...
  try {
    ...
    const [statusFieldId, pdfFieldId, tagId] = await Promise.all([
    ...
    return { ok: true, contactId, statusFieldId, pdfFieldId, tagId };
  } catch (err) {
