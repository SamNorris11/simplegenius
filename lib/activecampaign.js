...
// "Free Report Follow up" automation (built by Sam in the AC UI): email 1
// immediately, wait 3 days, email 2, wait 7 more days, email 3 — with a CTA
// in every email to schedule a call. Enrolling here (direct API call) means
// Sam never has to wire an AC-side tag trigger; the automation just needs to
// be left "Active" in the AC UI. schedule-confirmed.js removes the contact
// from this automation the moment they actually book a call.
const FOLLOWUP_AUTOMATION_ID = process.env.AC_FOLLOWUP_AUTOMATION_ID || '34';
...
/** Enter a contact into an automation directly via the API (bypasses AC-side
 * triggers entirely). Best-effort / idempotent-ish: AC allows a contact to
 * enter the same automation multiple times unless the automation's own
 * "enter only once" setting blocks it, so a duplicate call here is harmless
 * either way — it either starts a fresh run or AC itself declines the repeat. */
async function addContactToAutomation(contactId, automationId) {
  try {
    return await acRequest('POST', '/api/3/contactAutomations', {
      contactAutomation: { contact: contactId, automation: automationId },
    });
  } catch (err) {
    console.error(`AC addContactToAutomation(${automationId}) failed (non-blocking):`, err.message);
    return null;
  }
}

/** Remove a contact's active run of an automation by email — used so that a
 * contact who books a call stops receiving the rest of a follow-up sequence.
 * Never throws: this is called from a webhook path where a transient AC
 * hiccup must never fail the caller's own request. */
async function removeContactFromAutomation(email, automationId) {
  try {
    const config = getConfig();
    if (!config) return { ok: false, skipped: true };
    const url = `${config.baseUrl}/api/3/contactAutomations?automation=${encodeURIComponent(automationId)}&contact_email=${encodeURIComponent(email)}`;
    const res = await fetch(url, { method: 'DELETE', headers: { 'Api-Token': config.apiKey } });
    return { ok: res.ok, status: res.status };
  } catch (err) {
    console.error(`AC removeContactFromAutomation(${automationId}) failed (non-blocking):`, err.message);
...
async function syncBriefDelivered({ email, firstName, lastName, pdfUrl, company }) {
  ...
  try {
    ...
    // NOTE: this stores the raw PDF file URL, not the /report?src= viewer
    // URL that the active SendGrid "Brief Ready" email now uses (see
    // lib/brief-email-template.js) to get real new-tab link behavior for
    // citation links inside the PDF. If an AC automation is ever wired to
    // send its own delivery email off this field, route it through
    // /report?src=<encoded pdfUrl> the same way, and update
    // api/brief-redirect.js's ALLOWED_HOST_SUFFIX allowlist to accept
    // www.simplegenius.com in addition to the blob storage host.
    if (pdfUrl) await setFieldValue(contactId, pdfFieldId, pdfUrl);
    if (company) await setFieldValue(contactId, companyFieldId, company);
    // AC's automation-send personalization can read custom field values
    // slightly behind the field-write API (confirmed via test: %COMPANY%/
    // %BRIEF_PDF_URL% rendered blank/unresolved in the sent email even
    // though the fields were already correctly saved on the contact).
    // Give AC's data store a moment to catch up before the tag fires the
    // automation, so the "Send an Email" step personalizes with fresh data.
    await new Promise((resolve) => setTimeout(resolve, 5000));
    await tagContact(contactId, tagId);
    // Enter the contact into the "Free Report Follow up" automation. Never
    // lets a failure here affect the return value above — the brief is
    // already fully delivered and tagged by this point regardless.
    await addContactToAutomation(contactId, FOLLOWUP_AUTOMATION_ID);
