...
const { SITE_BASE_URLS } = require('./site');
...
function buildReportViewerUrl(pdfUrl) {
  // Reverted per Sam's explicit direction: link straight to the raw PDF,
  // no custom viewer page. Accepted tradeoff: source citation links inside
  // the PDF open in the same tab (native browser PDF viewer behavior).
  if (!pdfUrl) return '';
  return pdfUrl;
}

function renderBriefReadyEmail({ firstName, pdfUrl, email, site }) {
  const siteBaseUrl = SITE_BASE_URLS[site] || SITE_BASE_URLS.production;
  const safeFirstName = escapeHtml(firstName || 'there');
  // Link to our own pdf.js-based viewer instead of the raw PDF file. Chrome's
  // (and most browsers') native PDF viewer ignores target="_blank" on /URI
  // link annotations inside a PDF, so citation links inside the report would
  // navigate away in the same tab instead of opening in a new one. The
  // viewer at /report renders the PDF with pdf.js and forces every link to
  // a real HTML <a target="_blank"> element, which browsers honor.
  const viewerUrl = buildReportViewerUrl(pdfUrl);
  const safePdfUrl = escapeHtml(viewerUrl);
  const safeEmail = escapeHtml(email || '');

  const subject = 'Your free competitor report is ready';
...
<p style="margin:0 0 20px 0;font-family:Arial, Helvetica, sans-serif;line-height:26px;color:#1D1D1D;font-size:16px">Your free competitor report is ready.</p>
...
<a href="${safePdfUrl}" target="_blank" style="text-decoration:none;color:#D4AF37;font-size:16px;display:inline-block;padding:16px 36px;font-family:Arial, Helvetica, sans-serif;font-weight:bold;border-radius:4px">View Your Report</a>
</td></tr></table>
</td></tr>
...
<a href="${siteBaseUrl}" target="_blank" style="text-decoration:none;color:#1D1D1D;font-size:15px;display:inline-block;padding:14px 32px;font-family:Arial, Helvetica, sans-serif;font-weight:bold;border-radius:4px">Let's talk</a>
</td></tr></table>
</td></tr>
<tr><td style="padding:20px 40px;margin:0;background-color:#1D1D1D">
<p style="margin:0;font-family:Arial, Helvetica, sans-serif;line-height:18px;color:#8A8A8A;font-size:12px;text-align:center">Simple Genius &middot; The first step in AI for business<br><a href="${siteBaseUrl}" style="text-decoration:underline;color:#8A8A8A;font-size:14px">${siteBaseUrl.replace(/^https?:\/\//, '')}</a></p>
</td></tr>
</table>
</td></tr>
</table>
...
Your free competitor report is ready.
