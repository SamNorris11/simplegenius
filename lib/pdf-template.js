...
// Never let a long single-word category tag get forcibly split mid-word inside
// the rounded pill badge (e.g. "RESPONSIVENES" / "S" stacked on two lines).
// Tags are capped at 16 chars by the prompt, but we still scale the font down
// for longer tags so every tag always renders on one line, never broken.
function tagPillStyle(tag) {
  const len = ((tag || '')).length;
  let size;
  if (len <= 10) size = null; // use CSS default
  else if (len <= 12) size = '6.6pt';
  else if (len <= 14) size = '6pt';
  else size = '5.4pt';
  ...
  return `
      <div class="finding-card">
        <div class="finding-card-hdr">
          <span class="tag-pill"${tagPillStyle(f && f.tag)}>${esc(f && f.tag)}</span>
          ${sourcePill(f)}
        </div>
...
function renderCfindRow(f) {
  return `
      <div class="cfind-row">
        <div class="cfind-tag-col"><span class="tag-pill"${tagPillStyle(f && f.tag)}>${esc(f && f.tag)}</span></div>
...
function renderBriefHtml(job) {
  const siteBaseUrl = SITE_BASE_URLS[job && job.site] || SITE_BASE_URLS.production;
  ...
  // Company is already shown separately as the gold subtitle right above this
  // line (`.company-name`), so repeating it here read redundant. Prepared-for
  // now carries only the person's name and role.
  const preparedFor = role ? `${fullName}, ${role}` : fullName;
...
<div class="cover-inner">
  ...
  <div class="disclaimer-box">
    ...
    <div class="disclaimer-sub">No access to your systems. No conversations with your team.<br>Nothing but what anyone can find.</div>
  </div>
</div>
...
<div class="page interior">
  ...
  <div class="body-area">
    <div class="eyebrow">Insights worth your attention</div>
...
<div class="page close-page">
  <div class="body-area" style="padding-top:0;">
    <div class="close-title">What we found from the outside.</div>
    ...
    <a class="close-cta" href="${siteBaseUrl}" target="_blank" rel="noopener">See Simple Genius with the full picture &rarr;</a>
