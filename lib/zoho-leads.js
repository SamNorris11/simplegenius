...
const { SITE_TAGS, SITE_LABELS, siteSourceLine } = require('./site');
...
async function upsertZohoLead(record, tagNameOrNames) {
  ...
  // Accept either a single tag name (legacy callers) or an array of tag
  // names (e.g. the completion tag plus a site-attribution tag).
  const tagNames = (Array.isArray(tagNameOrNames) ? tagNameOrNames : [tagNameOrNames]).filter(Boolean);
  let tagAdded = false;
  for (const tagName of tagNames) {
    try {
      ...
      if (ok) tagAdded = true;
      if (!ok) {
        console.error(`Zoho lead tag (${tagName}) was not added:`, tagData?.data?.[0]?.code || tagRes.status);
      }
    } catch (tagErr) {
      console.error(`Zoho lead tag (${tagName}) error:`, tagErr.message);
    }
  }
...
function buildTryLeadRecord(body, site) {
  const siteKey = SITE_LABELS[site] ? site : 'production';
  const visitSummary = formatVisitSummary(
    body.visit_summary,
    'requesting the Try Simple Genius for Free brief'
  );
  const description = [
    siteSourceLine(siteKey),
  ...
  const record = {
    ...
    Prospect_Source_Detail: `Try Simple Genius for Free (${siteSourceLine(siteKey)})`
  };
...
async function syncTryLead(body, site) {
  const siteKey = SITE_LABELS[site] ? site : 'production';
  const record = buildTryLeadRecord(body, siteKey);
  return upsertZohoLead(record, ['try-simple-genius-free', SITE_TAGS.zoho[siteKey]]);
}
